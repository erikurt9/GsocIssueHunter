...
// Fallback to parsing root ISSUES.md when JSON is unavailable
...
export async function GET(request: NextRequest) {
  try {
    ...
    const gfi = searchParams.get("gfi") === "true";
    const gsoc = searchParams.get("gsoc") === "true";
    ...
    // Final fallback: parse local ISSUES.md from repo root
    if (!issues || issues.length === 0) {
      try {
        const readmePath = path.resolve(process.cwd(), "..", "ISSUES.md");
    ...
    if (label) {
      ...
      issues = issues.filter((i) => (i.labels || []).some((l) => (l || "").toLowerCase() === ll));
    }
    if (gfi) {
      issues = issues.filter((i) => !!i.es_good_first_issue);
    }
    if (gsoc) {
      issues = issues.filter((i) => !!i.es_gsoc_oficial);
    }
    ...
    const result = issues
      ...
      .map((i) => ({
        ...
        es_good_first_issue: !!i.es_good_first_issue,
        es_gsoc_oficial: !!i.es_gsoc_oficial,
      }));
