...
export default function HowItWorksPage() {
          ...
          <section id="overview" data-section ref={(el) => { observedRefs.current["overview"] = el; }}>
            ...
            <div className="space-y-8">
              <ProcessStep 
                ...
                description="Every day at midnight, our cron script discovers repositories from confirmed GSoC 2026 organizations, a curated community list, and a broad GitHub-wide search across any language or stack — not just JavaScript and not limited to repos with 'good first issue' labels."
              />
              ...
              <ProcessStep 
                ...
                description="For each discovered repository, we fetch all open, unassigned issues from the last few days — across every label, not only 'good first issue'. We tag which ones carry a good-first-issue label and which orgs are official GSoC 2026 orgs, so you can filter for either right in the app. We also collect repository metrics like stars, forks, and activity levels."
              />
        ...
        <CardContent className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <ul className="space-y-2 text-sm text-muted-foreground">
                        <li>• Repos from confirmed GSoC 2026 organizations</li>
                        <li>• Plus a curated list of community repositories</li>
                        <li>• Plus a broad GitHub-wide search — any language, stars &gt;20, pushed in the last month</li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2 text-sm text-muted-foreground">
                        <li>• Only open issues with no assignee</li>
                        <li>• Issue must have been created within the last 5 days</li>
                        <li>• Excludes issues with a linked PR or stale label</li>
                        <li>• Excludes issues where someone already claimed interest in the thread</li>
                        <li>• "Good first issue" and "GSoC official" are tagged, not required — filter for them in the app</li>
                      </ul>
                    </div>
                  </div>
                </CardContent>
  ...
  // No 'labels' param here — we fetch ALL open issues, not just
  // "good first issue" ones. GFI is only recorded as a boolean flag below.
  const cutoff = new Date(Date.now() - MAX_DIAS_CREACION * 24 * 60 * 60 * 1000);

  return response.data.filter(issue => {
    if (issue.pull_request) return false;
    if (new Date(issue.created_at) < cutoff) return false;
    if (issue.assignee || issue.assignees?.length) return false;
    return true; // unassigned, recent, open — regardless of label
  }).map(issue => {
    const labels = (issue.labels || []).map(l => l.name.toLowerCase());
    issue.es_good_first_issue = labels.some(l => GOOD_FIRST_ISSUE_REGEX.test(l));
    issue.es_gsoc_oficial = repo._fuente === 'gsoc' || labels.some(l => GSOC_LABEL_REGEX.test(l));
    return issue;
  });
};`}
                    />
